...
#u_0 = 0.5*jnp.exp(-100 * (mesh - DOMAIN_SIZE / 2)**2) # 2
#u_0 = jnp.sin(2 * jnp.pi * mesh / DOMAIN_SIZE) + 0.5 * jnp.sin(4 * jnp.pi * mesh / DOMAIN_SIZE) + 0.25 * jnp.sin(8 * jnp.pi * mesh / DOMAIN_SIZE) #-low
...
with h5py.File(f"data/{u0_idx}_noiseless_generated.h5", "w") as f:
...
# while j < 2000:
# 	sigma = jnp.std(u_0)
# 	u_noise = u_0 + 0.01*sigma*jax.random.normal(jax.random.PRNGKey(j), shape=mesh.shape)
